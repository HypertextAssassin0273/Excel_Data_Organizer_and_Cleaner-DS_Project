...
public:
...
#if __cplusplus >= 201103L
	Vector(const std::initializer_list<char>& list)noexcept://i.e. initializer-list ctor
	...
	template<__uint64 n>
	Vector(const char (&str)[n])noexcept://i.e. copy ctor for taking const char* type string
...
public:
	...
	template<__uint64 n>
	Vector operator+(const char (&str)[n])const{
		Vector temp=*this;
